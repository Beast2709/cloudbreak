...
clean_up_fstab() {
    ...
    valid_uuids_expr=$(blkid | sed 's/^.*UUID=\"\(.*\)\"\sTYPE=.*$/\1/' | tr '\n' "|")
    sed -i -E "/${valid_uuids_expr%?}/!d" /etc/fstab
}
...
main () {
    ...
    readarray -t device_uuids_arr < <(blkid | sed -nE 's/(\/dev\/nvme0n1|\/dev\/nvme1n1):\sUUID="(.*)"\sTYPE="ext4"/\2/p' | tr '\n' ' ')
